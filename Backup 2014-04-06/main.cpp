...
const byte bronzeCypher[] = {0x0f, 0xf6, 0xa3, 0x62, 0xa5, 0xb5, 0x0f, 0xca, 0x0f, 0x92, 0x27, 0xca, 0x13, 0x62, 0x14, 0xbb, 0xb5, 0xc0, 0xbf, 0x81, 0xca, 0x89, 0x20, 0x89, 0xa4, 0x89, 0x89, 0xc7, 0x89, 0x4b};
const int bronzeCypherSize = 30;
const byte silverCypher[] = {0xd0, 0xae, 0x4f, 0x7c, 0xcf, 0xea, 0xa6, 0x2d, 0xd8, 0xe8, 0x4f, 0x50, 0x7d, 0xcf, 0xea, 0xd8, 0x42, 0x8d, 0x42, 0x42, 0xd8, 0x42, 0x8d, 0x24, 0xf3, 0xd8, 0xf3, 0x5c, 0xcc, 0x7d};
const int silverCypherSize = 30;
...
byte TripleWESRoundDecrypt[0x1000000];
byte TripleWESRoundEncrypt[0x1000000];
byte enc1, enc2, enc3, enc4, dec1, dec2, dec3, dec4, diff1, diff2, diff3, ddiff1, ddiff2, ddiff3;
...
void init () {
    ...
    int k1, k2, k3;
    for (k1 = 0x00; k1 <= 0xFF; k1++) {
        for (k2 = 0x00; k2 <= 0xFF; k2++) {
            for (k3 = 0x00; k3 <= 0xFF; k3 ++) {
                TripleWESRoundEncrypt[(k1 << 16) | (k2 << 8) | k3] = WESRoundEncrypt[WESRoundEncrypt[WESRoundEncrypt[k1] ^ k2] ^ k3];
                TripleWESRoundDecrypt[(k1 << 16) | (k2 << 8) | k3] = WESRoundDecrypt[WESRoundDecrypt[WESRoundDecrypt[k1] ^ k2] ^ k3];
            }
        }
    }
}
...
int main() {
    ...
    for (k1 = 0x000000; k1 < 0x1000000; k1 ++) {
        enc1 = TripleWESRoundEncrypt[0x410000 ^ k1];// ^ k4;
        enc2 = TripleWESRoundEncrypt[0x420000 ^ k1];// ^ k4;
        enc3 = TripleWESRoundEncrypt[0x430000 ^ k1];// ^ k4;
        enc4 = TripleWESRoundEncrypt[0x440000 ^ k1];// ^ k4;

        i = (enc1 ^ enc2) | ((enc2 ^ enc3) << 8) | ((enc3 ^ enc4) << 16);
        val = (k1 << 0); // | k4;
    ...
    for (k5 = 0x00; k5 <= 0xFF; k5++) {
        for (k6 = 0x00; k6 <= 0xFF; k6++) {
            for (k7 = 0x00; k7 <= 0xFF; k7++) {
                for (k8 = k7; k8 <= k7; k8++) {
                    dec1 = WESRoundDecrypt[
                            WESRoundDecrypt[
                            WESRoundDecrypt[
                    ...
                    d = (dec1 ^ dec2) | ((dec2 ^ dec3) << 8) | ((dec3 ^ dec4) << 16);
                    start = fStart[d];

                    while (start) {
                        k1 = (byte) ((0xFF0000 & (*start)) >> 16);
                        k2 = (byte) ((0xFF00 & (*start)) >> 8);
                        k3 = (byte) ((0xFF & (*start)) >> 0);
                        k4 = WESRoundEncrypt[
                            WESRoundEncrypt[
                            WESRoundEncrypt[
                                0x41 ^ k1
                        ...
                        if (!error) {
                            foundKeys.push_back({k1, {k2, k3, k4}, {k5, k6, k7}, {k8, k8, k8}});
                        }
    ...
    for (int i = 0; i < (int) foundKeys.size(); i++) {
        cout << "Key " << getHex(foundKeys[i]) << ": " << endl;
        for (int j = 0; j < bronzeCypherSize; j++) {
            cout << TripleWESDecrypt(bronzeCypher[j], foundKeys[i]);
        }
        cout << endl << endl;
    }
